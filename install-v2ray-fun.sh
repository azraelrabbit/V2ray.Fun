...
stopV2ray(){
    colorEcho ${BLUE} "Shutting down V2Ray-fun service."
    if [[ -n "${SYSTEMCTL_CMD}" ]] || [[ -f "/lib/systemd/system/v2ray-fun.service" ]] || [[ -f "/etc/systemd/system/v2ray-fun.service" ]]; then
...
startV2ray(){
    if [ -n "${SYSTEMCTL_CMD}" ] && [ -f "/lib/systemd/system/v2ray-fun.service" ]; then
        ${SYSTEMCTL_CMD} start v2ray
    elif [ -n "${SYSTEMCTL_CMD}" ] && [ -f "/etc/systemd/system/v2ray-fun.service" ]; then
        ${SYSTEMCTL_CMD} start v2ray
    elif [ -n "${SERVICE_CMD}" ] && [ -f "/etc/init.d/v2ray" ]; then
...
installV2Ray(){
    ...
    if [[ ! -f "/etc/v2ray/config-fun.json" ]]; then
        mkdir -p /etc/v2ray
        mkdir -p /var/log/v2ray
        cp "/tmp/v2ray/v2ray-${NEW_VER}-linux-${VDIS}/vpoint_vmess_freedom.json" "/etc/v2ray/config-fun.json"
        if [[ $? -ne 0 ]]; then
        ...
        sed -i "s/10086/${PORT}/g" "/etc/v2ray/config-fun.json"
        sed -i "s/23ad6b10-8d1a-40f7-8ad0-e3e35cd38297/${UUID}/g" "/etc/v2ray/config-fun.json"
...
installInitScript(){
    if [[ -n "${SYSTEMCTL_CMD}" ]];then
        if [[ ! -f "/etc/systemd/system/v2ray-fun.service" ]]; then
            if [[ ! -f "/lib/systemd/system/v2ray-fun.service" ]]; then
                cp "/tmp/v2ray/v2ray-${NEW_VER}-linux-${VDIS}/systemd/v2ray.service" "/etc/systemd/system/v2ray-fun.service"
                sed s/"PIDFile=/var/run/v2ray.pid"/"PIDFile=/var/run/v2ray-fun.pid"/ "/etc/systemd/system/v2ray-fun.service"
                sed s/"ExecStart=/usr/bin/v2ray/v2ray -config /etc/v2ray/config.json"/"ExecStart=/usr/bin/v2ray/v2ray -config /etc/v2ray/config-fun.json"/ "/etc/systemd/system/v2ray-fun.service"
                systemctl enable v2ray-fun.service
            fi
        fi
        return
    elif [[ -n "${SERVICE_CMD}" ]] && [[ ! -f "/etc/init.d/v2ray" ]]; then
...
remove(){
    if [[ -n "${SYSTEMCTL_CMD}" ]] && [[ -f "/etc/systemd/system/v2ray-fun.service" ]];then
        if pgrep "v2ray" > /dev/null ; then
            stopV2ray
        fi
        systemctl disable v2ray.service
        rm -rf "/usr/bin/v2ray" "/etc/systemd/system/v2ray-fun.service"
        if [[ $? -ne 0 ]]; then
